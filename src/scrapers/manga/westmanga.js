...
class WestmangaScraper extends BaseScraper {
  ...
  async scrape(url) {
    try {
      ...
      const coverElem = $('img[itemprop="image"][title]');
      ...
      // WestManga does not typically provide release date in a consistent way
      data.releaseDate = '';

      // Extract author from table
      const authorElem = $('td:contains("Author") + td');
      data.author = authorElem.length ? authorElem.text().trim() : '';

      // Extract artist from table
...
module.exports = WestmangaScraper;