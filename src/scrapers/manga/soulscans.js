...
class SoulScansScraper extends BaseScraper {
  ...
  async scrape(url) {
    try {
      ...
      const altTitleElem = $('span.alternative');
      ...
      // Extract description
      const descElem = $('div.entry-content.entry-content-single[itemprop="description"]');
      if (descElem.length) {
        // Clean up the description - remove links and keep only paragraphs
        descElem.find('a').remove();
        let description = '';
        descElem.find('p').each((i, elem) => {
      ...
      // SoulScans typically has ongoing series
      data.status = 'Ongoing';
      ...
      const releaseElem = $('div.imptdt:contains("Released") i');
      data.releaseDate = releaseElem.length ? releaseElem.text().trim() : '';

      // Extract author
      const authorElem = $('div.imptdt:contains("Author") i');
      data.author = authorElem.length ? authorElem.text().trim() : '';

      // Extract artist
      const artistElem = $('div.imptdt:contains("Artist") i');
      data.artist = artistElem.length ? artistElem.text().trim() : '';

      // Extract genres
      const genresElem = $('div.wd-full span.mgen');
...
module.exports = SoulScansScraper;