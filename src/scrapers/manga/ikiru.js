...
class IkiruScraper extends BaseScraper {
  ...
  async scrape(url) {
    try {
      ...
      const releaseElem = $('div.fmed:contains("Released") span');
      data.releaseDate = releaseElem.length ? releaseElem.text().trim() : '';

      // Scraping author
      const authorElem = $('div.fmed:contains("Author") span');
      if (authorElem.length) {
        let authorText = authorElem.text().replace('[Add, ]', '').replace('&nbsp;', ' ').trim();
      ...
      // Scraping artist
      const artistElem = $('div.fmed:contains("Artist") span');
      if (artistElem.length) {
        let artistText = artistElem.text().replace('[Add, ]', '').replace('&nbsp;', ' ').trim();
