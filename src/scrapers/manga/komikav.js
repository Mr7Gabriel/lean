...
class KomikavScraper extends BaseScraper {
  ...
  async scrape(url) {
    try {
      ...
      // Extract title from image title attribute
      const titleImg = $('img[itemprop="image"]');
      if (titleImg.length && titleImg.attr('title')) {
        data.title = titleImg.attr('title').trim();
      } else {
        // Try alternate method
        const titleElem = $('h1.title');
      ...
      if (descElem.length) {
        // Remove HTML tags but maintain paragraph breaks
        let description = '';
